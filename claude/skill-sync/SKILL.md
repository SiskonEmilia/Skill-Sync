...
metadata:
  audience: all
  instructions: |
    This file is the Claude Code-adapted version generated from src/skill.md.
...
${CLAUDE_SKILL_DIR}/ → ~/skill-sync/claude/<skill>/
~/.config/opencode/skills/<skill>/ --junction--> ~/skill-sync/opencode/<skill>/
```

You can reference the current skill directory with `${CLAUDE_SKILL_DIR}`.
...
2. Identify which CLI it belongs to (check if it's in `${CLAUDE_SKILL_DIR}` or `~/.config/opencode/skills/`)
...
- No need to add Claude-specific fields unless the skill requires them
- Add: `disable-model-invocation: true` (skill-sync operations are side-effectful and should only be user-invoked)
...
1. Run !`git -C ~/skill-sync fetch` and check for merge conflicts
...
6. On the other device: !`git -C ~/skill-sync pull` (no junction changes needed since junction targets are file-level)
